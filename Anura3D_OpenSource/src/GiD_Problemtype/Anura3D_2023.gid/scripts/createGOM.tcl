  ...
  GiD_WriteCalculationFile puts "### Anura3D_2023 ###"
	...
	if {$dim_type == "3D" || $dim_type == "3D:Axissymmetric"} {
	 ...
	 if {$type == "yes"} {
		...
		set type [$node getAttribute "v"]
		                        GiD_WriteCalculationFile puts {$$UNDRAINED_POISSON_RATIO} 
		                                GiD_WriteCalculationFile puts $type
		 set node [$gNode selectNodes [format_xpath {container[@n="_material_constitutive_model"]/value[@n="un_Young_modulus_"]}]]
		set type [$node getAttribute "v"]
		                        GiD_WriteCalculationFile puts {$$UNDRAINED_YOUNG_MODULUS} 
		                                GiD_WriteCalculationFile puts $type
		} elseif {$typename == "Saturated material-undrained total stress"} {
		set node [$gNode selectNodes [format_xpath {container[@n="_material_constitutive_model"]/value[@n="un_poisson_ratio_"]}]]
		set type [$node getAttribute "v"]
